...
from pyspark.sql.functions import window, count, col, from_json, to_timestamp, current_timestamp
from pyspark.sql.types import StructType, StructField, StringType, TimestampType
import sys
import os
from datetime import datetime
from pyspark.sql.functions import window, count, col, from_json, to_timestamp, current_timestamp, coalesce
...
def create_or_update_cumulative_df(new_counts_df):
    ...
    if emoji_cumulative_counts_df is None:
        ...
        emoji_cumulative_counts_df = emoji_cumulative_counts_df \
            .join(new_counts_df, on="emoji_type", how="outer") \
            .select(
                "emoji_type", 
                (coalesce(emoji_cumulative_counts_df["emoji_count"], F.lit(0)) + coalesce(new_counts_df["emoji_count"], F.lit(0))).alias("emoji_count"))
    emoji_cumulative_counts_df = emoji_cumulative_counts_df \
        .withColumn(
            "emoji_aggregated_count",
            when(col("emoji_count") < 1000, 1)
            .otherwise((col("emoji_count") / 1000).cast("int"))
        )
...
def process_batch(df, epoch_id):
    try:
        ...
        # Apply scaling
            # (col("emoji_count") / 1000).cast("int") + 1
        # scaled = processed.withColumn(
        #     "emoji_aggregated_count",
        #     when(col("emoji_count") < 1000, 1).otherwise((col("emoji_count") / 1000).cast("int"))
        # )
        
        emoji_cumulative_counts_df = create_or_update_cumulative_df(processed)
        ...
        if cumulative_data:
            # Write the cumulative counts to Kafka
            spark \
                .createDataFrame([(cumulative_data,)], ["value"]) \
                .write \
                .format("kafka") \
                .option("kafka.bootstrap.servers", "localhost:9092") \
                .option("topic", "processed_emojis") \
